 ...
 * <drashna@live.com> Copyright 2019 Sunjun Kim
 *
...
typedef union {
  uint16_t raw;
  struct {
    uint16_t pointer_default_dpi : 10;
    uint16_t pointer_sniping_dpi : 1;
...
static uint16_t get_pointer_default_dpi(charybdis_config_t *config) {
  return (uint16_t)config->pointer_default_dpi *
...
static uint16_t get_pointer_sniping_dpi(charybdis_config_t *config) {
  return (uint16_t)config->pointer_sniping_dpi *
...
#ifdef MACCEL_AFFECTS_DRAGSCROLL
/**
 * \brief Augment the pointing device behavior.
 *
 * Maccel-implementation of drag-scroll.
 * This implementation applies the accel curve to dragscroll as well.
 * If you prefer the default charybdis implementation, remove the
 * MACCEL_AFFECTS_DRAGSCROLL definition from your config.h
 */
 ...
 * \brief Augment the pointing device behavior.
 *
 * Default Charybdis implementation of drag-scroll.
 */
static void pointing_device_task_charybdis(report_mouse_t *mouse_report) {
...
#endif // MACCEL_AFFECTS_DRAGSCROLL
...
void keyboard_pre_init_kb(void) {
   ...
   */
  const pin_t unused_pins[] = {GP1,  GP3,  GP4,  GP6,  GP8,  GP10,
                               GP14, GP16, GP18, GP21, GP22, GP24,
